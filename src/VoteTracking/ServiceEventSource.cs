...
{
	[EventSource(Name = "Pri-ServiceApplication-VoteTracking")]
...
#if !HEALTH_DISABLED
			public const EventKeywords HealthReport = (EventKeywords) 0x4L; // PR
#endif
		}

		#region default event-source implementation
...
SizeInBytes(applicationTypeName) };
		...
		#endregion default event-source implementation

#if !HEALTH_DISABLED
		// PR
		private const int HealthReportEventId = 10;

		[Event(HealthReportEventId, Level = EventLevel.LogAlways,
			Message = "Health report. Source '{0}' property {1} is {2}. Partition: {3}, Instance or Replica: {4} Desc: {5}.",
			Keywords = Keywords.HealthReport)]
		public void HealthReport(string healthSourceId, string name, string state, Guid partition, long instanceOrReplica,
			string description)
		{
		...
		[Event(HealthReportIntervalChangedEventId, Level = EventLevel.Informational,
			Message =
				"Health report interval changed to {4} seconds for {0} property {1}. Partition: {2} Instance or Replica: {3}.",
			Keywords = Keywords.HealthReport)]
		public void HealthReportIntervalChanged(string healthSourceId, string name, Guid partition, long instanceOrReplica,
			int duration)
		{
